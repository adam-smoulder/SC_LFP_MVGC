...
momax     = 80;     % maximum model order for model order estimation
...
if strcmp(cueString,'targlfp')
    X = reshape([dataToUse.targcsd],[size(dataToUse(1).sacccsd) nTrials]);
elseif strcmp(cueString,'sacclfp')
    X = reshape([dataToUse.sacccsd],[size(dataToUse(1).sacccsd) nTrials]);
else
...
quartile = size(X,1)/4;
X = [mean(X(1:quartile,:,:));...
    mean(X((quartile+1):2*quartile,:,:));...
    mean(X((2*quartile+1):3*quartile,:,:))];
...
clearvars -except AIC BIC cueString dnobs doHP doNotch DS dur fname freqs fres...
    ...
    intChans deepChans trialNums origSpecGC origTimeGC ...
    nullDistChoice endPart totalTime nullDistSpecGC

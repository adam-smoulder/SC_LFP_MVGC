...
maxPlotF = 500;             % max frequency to display
...
% find mean sgram
for i = 1:lChanPlot
    Smean(i,:,:) = abs(spectrogram(squeeze(meanTraces(i,:)), windowSize, nOverlap, nFFT, rawFs));
    disp(['Calculated sgram # ' num2str(i)])
end
...
%     hold off
    imagesc(sgramTime, sgramFreqs, squeeze(Smean(i,:,:)))
    ylabel(['Chan ' num2str(channelLabels(i))]);
    colormap jet
    set(gca, 'CLim', [0,maxSgramPlotMag/5]);
    axis xy
    axis([-inf inf 0 maxPlotF])
    if i == 1
        title(['Mean LFP'])

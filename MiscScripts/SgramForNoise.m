...
maxSgramPlotMag = 500;        % defaults to actual max
maxLfpPlotMag = 0;          % defaults to actual max
maxPlotF = 15000;             % max frequency to display
nFFT = 8092*8;              % length of DFT
pointsPerEval = 300;        % step for center of shifting window
        
windowSize = rawFs/10;       % size of window
...
% end
    ...
    % plot sgram
    imagesc(sgramTime, sgramFreqs, squeeze(S(1,:,:)))
    ylabel(['Chan ' num2str(channelLabels(1))]);
    colormap jet
    set(gca, 'CLim', [0,maxSgramPlotMag]);
    axis xy
    axis([-inf inf 0 maxPlotF])

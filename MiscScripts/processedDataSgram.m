...
maxPlotF = 250;             % max frequency to display
nFFT = 8092*8;              % length of DFT
pointsPerEval = 10;        % step for center of shifting window
        
windowSize = fs/8;       % size of window
...
lTime = ceil(((fs+1)-windowSize)/pointsPerEval); % size of time output for sgram
...
for i = 1:lChanPlot
    ...
    eval(evalString);
    S(i,:,:) = 20*log10(abs(spectrogram(squeeze(trialLfp(i,:)), windowSize, nOverlap, nFFT, 1000)));

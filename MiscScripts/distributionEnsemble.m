scenario = 2;
numsep = 2;
% trial ensemble is moot; it's zero average!
...
tempSpecGC = zeros([numsep size(specGC)]);
tempTimeGC = zeros([numsep size(timeGC)]);
...
splits = linspace(1, size(tempX,3), numsep+1);
...
if scenario == 2  % distribution ensemble
    specGC = squeeze(mean(tempSpecGC,1));
    timeGC = squeeze(mean(tempTimeGC,1));
end
...
clearvars -except AIC BIC cueString dnobs doHP doNotch DS dur fname freqs fres...
    ...
    intChans deepChans tempSpecGC tempTimeGC

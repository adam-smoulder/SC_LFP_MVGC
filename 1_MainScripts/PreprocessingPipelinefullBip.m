...
fname = 'bl_sc_112515_mcell_spikelfp_cSC.mat';  % dataset file name
...
pairs = [1 3; 2 4; 5 7; 6 8; 9 11; 10 12; 13 15; 14 16];


nTrials = length(data);
for trial = 1:nTrials    
    % perform bipolar subtraction
    for j=1:length(pairs) % for each pair of channels
...
if detrend
    ...
    tempTargData = zeros([size(data(1).targlfpbip) nTrials]);
    tempSaccData = zeros([size(data(1).sacclfpbip) nTrials]);
    tempTargData = homeDetrend(reshape([data.targlfpbip],size(tempTargData)));
    tempSaccData = homeDetrend(reshape([data.sacclfpbip],size(tempSaccData)));
...
meanTargLFPPostDetrend = mean(reshape([data.targlfpmat],[8, 30001, length(data)]),3);
meanSaccLFPPostDetrend = mean(reshape([data.sacclfpmat],[8, 30001, length(data)]),3);
...
outputFileName = strcat('',fname(1:end-23),'_fullBip.mat');
